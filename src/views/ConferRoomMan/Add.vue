<template>
  <el-dialog :title="title" :visible="openAdd" v-el-drag-dialog v-on:update:visible="close($event)">
    <el-form :model="form" :rules="formRules" ref="addForm">
      <el-form-item prop="code" label="会议室编码" label-width="120px" required>
        <el-input v-model="form.code"></el-input>
      </el-form-item>
      <el-form-item prop="name" label="会议室名称" label-width="120px" required>
        <el-input v-model="form.name"></el-input>
      </el-form-item>
      <el-form-item prop="capacity" label="容纳人数" label-width="120px" required>
        <el-input type="number" v-model="form.capacity"></el-input>
      </el-form-item>
      <el-form-item prop="introduce" label="会议室简介" label-width="120px">
    ...
    return {
      ...
      formRules: {
        code: [
          {
            trigger: "blur",
            validator: validateInput("会议室编码")(validInput)
          }
        ],
        name: [
          {
            trigger: "blur",
            validator: validateInput("会议室名称")(validInput)
          }
        ],
        introduce: [
          {
            trigger: "blur",
            validator: validateInput("会议室简介")(validTextarea)
          }
        ]
      }
    };
  ...
  methods: {
    ...
    save() {
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$store
            .dispatch("conferRoom/" + this.requestPath, this.form)
            .then(() => {
              this.$message({
                type: "success",
